...
/*/ :::::::::::::::::  LUC version  ::::::::::::::::
double SimpleUniverse::LUC_version = 1.6;
// -- August 2007 : The specific methods for calculating the CMB sound horizon  
//                  are put in a separate class (SUniverseSHRec)

// ::::::::::::::::::::::::::::::::::::::::::::::::::
// ::::::::::::  Constants and parameters ::::::::::::
double SimpleUniverse::MpctoMeters_Cst = 3.0856e22;  // Mpc in meters 
double SimpleUniverse::YeartoSec_Cst = 3.155760e7;   // seconds in a year
double SimpleUniverse::SpeedOfLight_Cst = 2.99792458e8;  // metres/sec
double SimpleUniverse::LightYear_Cst = 9.46073042e15;   // 1 Light-Year in meters
double SimpleUniverse::G_Newton_Cst = 6.673e-11;        // G_N in SI units 
double SimpleUniverse::h_Planck_Cst = 6.62606876e-34;   // Planck constant SI units
double SimpleUniverse::k_Boltzman_Cst = 1.3806503e-23;  // Boltzman constant SI (J K^-1)
double SimpleUniverse::k_BoltzmaneV_Cst = 8.617342e-5;  // Boltzman constant in eV (electron-volt)
double SimpleUniverse::ElectronCharge_Cst = 1.602176462e-19; // electric charge in SI units 
double SimpleUniverse::ElectronMassGeV_Cst = 0.510998902e-3; // Electron mass in GeV 
double SimpleUniverse::ProtonMassGeV_Cst = 0.938271998;      // Proton mass in GeV

// 3H0/8piG_N H0=100 km/s/Mpc 
double SimpleUniverse::RhoCrit_h1_Cst = 1.879e-26;      // Kg/m^3
double SimpleUniverse::RhoCrit_h1_MSolMpc3_Cst = 2.77536627e11; // SolarMass/Mpc^3
double SimpleUniverse::RhoCrit_h1_GeVcm3_Cst = 1.0539e-5;  // in GeV/cm^3

// Universe parameters
double SimpleUniverse::T_CMB_Par = 2.725;             // CMB temperature today
double SimpleUniverse::rho_gamma_Par = 4.6417e-31;    // Photon density today (kg/m^3) 
double SimpleUniverse::numberdens_gamma_Par = 410.4;  // Photon number density today /cm^3 
double SimpleUniverse::rho_gamma_nu_Par = 7.8042e-31; // Photon+neutrino density (kg/m^3) 
double SimpleUniverse::Z_Rec_Par = 1088.;              // Value in WMAP ApJ paper
// ::::::::::::::::::::::::::::::::::::::::::::::::::*/
...
/* --Method-- */
void SimpleUniverse::Seth(double h)
{
  h_ = h;
  ComputeCurvature();
}
...
void SimpleUniverse::Print(ostream & os) const 
{
  ...
  if (OmegaPhoton() > 1.e-39) 
    os << " Zequ= " << Z_Equality() << endl;
  else os << " Zequ= ???? " << endl;
  if (hasX_)
	os << " OmegaDE= " << OmegaDE();
  else
  	os << " OmegaL= " << OmegaLambda();
						...
						double& resG, double& resGT)
{
   ...
   /* -----  Utilisation of the precision to calculate the step - 
   We use the relation :
   ...
   */
   ...
   // Integration with fixed step up to z=zbrk (=10.)
   double zbrk = 10.;
   ...
   if (z1<zbrk) {    // Integration up to zbrk 
     ...
     accG *= dz;   accGT *= dz;
     dz = z2-zc;
     if (dz > 1.e-69) {       // Correction for the last step eventuel 
...
inline double puiss_2(double x) { return x*x; }
inline double puiss_3(double x) { return x*x*x; }
inline double puiss_4(double x) { return x*x*x*x; }
inline double puiss_5(double x) { return x*x*x*x*x; }
...
double SimpleUniverse::IntegrateGz_NoLambda(double z)
{
  return (
   ...
   omegarad_*puiss_2(z)))/(1 + z)))/
   (sqrt(omegaCurv_)*sqrt(omegaCurv_*puiss_2(1 + z) + 
   omegamat_*puiss_3(1 + z) + omegarad_*puiss_4(1 + z))))
   );
}
...
double SimpleUniverse::IntegrateGTz_NoLambda(double z)
{
  return (
	  -((omegaCurv_ + omegamat_ + omegarad_ + omegamat_*z + 
	     2*omegarad_*z + omegarad_*puiss_2(z)) /
	    ( omegaCurv_*sqrt(omegaCurv_*puiss_2(1+z) + 
			      omegamat_*puiss_3(1+z) + omegarad_*puiss_4(1+z)) ) ) + 
	  (omegamat_*(1+z)*sqrt(omegaCurv_+omegamat_ + omegarad_+omegamat_*z + 
				2*omegarad_*z + omegarad_*puiss_2(z)) *
	   log((-2.*(2.*puiss_2(omegaCurv_) + omegaCurv_*omegamat_ + 
		     omegaCurv_*omegamat_*z)) / (sqrt(omegaCurv_)*omegamat_*(1+z)) - 
	       (4*omegaCurv_*sqrt(omegaCurv_+omegamat_+omegarad_+omegamat_*z + 
				  2*omegarad_*z + omegarad_*puiss_2(z))) / 
	       (omegamat_*(1+z)) )  ) / 
	  (2.*pow(omegaCurv_,1.5)*sqrt( omegaCurv_*puiss_2(1+z) + 
				        omegamat_*puiss_3(1+z) + 
				        omegarad_*puiss_4(1+z) ) )
...
{
  return ( -(2*omegaCurv_*(1+z) + 3*omegamat_*puiss_2(1+z) 
	     + 4*omegarad_*puiss_3(1+z)) / 
	   (2.*pow(omegaCurv_*puiss_2(1+z) 
		   + omegamat_*puiss_3(1+z) 
		   + omegarad_*puiss_4(1+z),1.5)) ) ;
}    
...
double SimpleUniverse::DerivateGz_Lambda(double z)
{
  return ( -(2.*omegaCurv_*(1+z) + 3.*omegamat_*puiss_2(1+z) + 
	     4.*omegarad_*puiss_3(1+z)) /
	   (2.*pow(omegaL_ + omegaCurv_*puiss_2(1+z) + 
		   omegamat_*puiss_3(1+z) + omegarad_*puiss_4(1+z), 1.5) ) );
}
...
double SimpleUniverse::DerivateGz_Lambda_X(double z)
{
  ...
  return ( -(2*omegaCurv_*(1+z) + 3*omegamat_*puiss_2(1+z) 
	   ...
	   (2.*pow(omegaL_+omegaCurv_*puiss_2(1+z) + omegamat_*puiss_3(1+z) 
		   + omegarad_*puiss_4(1+z) + 
