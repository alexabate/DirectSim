...
/** SimpleUniverse class
  ...
  *
  * Constructor is a function of the cosmological parameters
  *
  */
class SimpleUniverse {
public:
  
    /** Default constructor, default parameters: h = 0.719, Omega_B = 0.0441,
        Omega_M = 0.214*Omega_B, Omega_L = 0, flat universe                   */
    SimpleUniverse();
  
    /** Constructor for a universe with just one matter component determined
        by omega0. Omega_L = 0, Omega_B = 0.044, curvature is calculated. 
        @param h        Hubble parameter in units of 100 km/s/Mpc (H0 = 100h km/s/Mpc)
        @param omega0   Total matter density, in units of the critical density */
    SimpleUniverse(double h, double omega0);
  
    /** Constructor for a universe composed of matter component (omega0) and 
        lambda (omegaL). Omega_B = 0.044, curvature is calculated. 
        @param h        Hubble parameter in units of 100 km/s/Mpc (H0 = 100h km/s/Mpc)
        @param omega0   Total matter density, in units of the critical density
        @param omegaL   Cosmological constant density in units of the critical density*/
    ...
    void  Seth(double h) {
        h_ = h;
        ComputeCurvature(); // think call this so all quantities are set to 0
        };
    ...
    /** Set dark energy, Omega_DE and equation of state
        @param omegaX   Dark energy density in units of the critical density
        @param wX       Dark energy equation of state                         */
    ...
    /** Set flat universe using Omega_M (Omega_Lambda = 1 - Omega_M)          */
    void  SetFlatUniverse_OmegaMatter();
    
    /** Set flat universe using Omega_Lambda (Omega_M = 1 - Omega_Lambda)     */
    ...
    /** Defines the Emission redshift then computes the corresponding 
        ...
        while. 
        @param ze       emission redshift
        @param prec     target precision used to compute the adaptative integration step
        @param fginc    if true, does incremental calculation of integrals    */
    ...
    /** Return equivalent error in comoving distance to error in redshift 
    ...
    /** Print cosmological parameters and quantities evalulated at the current
        emission redshift to specified stream           
        @param os   output stream                                             */
    void          Print(ostream & os) const ;
    /** Print cosmological parameters and quantities evalulated at the current
        emission redshift to screen                                           */
    ...
    void NumIntegrateGzGTz(double z1, double z2, double prec, double& resG, double& resGT);
    ...
    void NumIntegVolElz(double z1, double z2, int nstep, double& dVdO);
...
protected:
    /** Set parameters to some default values: h = 0.5, Omega_M = 1., all other
        components are zero. All cosmological quantities set to today's values*/
    void          Initialize();
    ...
    @param resG Resulting integration of G(z)
    @param resGT Resulting integration of GT(z)                               */
    void NumIntegGzGTz(double z1, double z2, double prec, double& resG, double& resGT);
