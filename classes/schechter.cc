...
// Constructor: LF pars directly
Schechter::Schechter(double phistar,double Mstar,double alpha)
...
Schechter::Schechter(void)
  : phistar_(0.) , Mstar_(0.) , alpha_(0.) , outvalue_(0)
{
}
...
void Schechter::SetParam(double phistar,double Mstar,double alpha)
{
  phistar_ = phistar; Mstar_ = Mstar; alpha_ = alpha;
}

void Schechter::GetParam(double& phistar,double& Mstar,double& alpha)
{
  ...
  , hmdndm_(NULL) , tirhmdndm_(NULL)
{
  ...
  // Creation of random drawing according to Schechter
  sch_outvalue_ = sch.GetOutValue();  // save the configuration of the schechter
  sch.SetOutValue(1);  // want m*dN/dm
  double lnx1 = log10(massmin_), lnx2 = log10(massmax_); // Binning in log10 of the mass
  if(nbinmass_<0) 
...
double SchechterMassDist::TirMass(int ngal)
{
 ...
 if(ipo<0) {  // No drawing histos for this number of galaxies
   for(long i=0;i<ngal;i++) {  // Draw ngal times from the Schechter
     // double lm = tirhmdndm_->Random();
     double lm = tirhmdndm_->RandomInterp();
     masse_des_ngal += pow(10.,lm);  // ATTENTION abscissa in log10(mass)
   }
   ntrial_dir++;
 } else {
   // ATTENTION the drawing histo stores the log10(mean=sum_masses/ngal)
