...
// 			linear extrapolate outside range:
				return ( yreg_[0]+(x-xmin_)*
						(yreg_[1]-yreg_[0])/dx_ ); 
		...
		if (i>=(int)npoints_)
			{
			if(!setzero_)// if not automatically setting zero ...
				return ( yreg_[npoints_]+(x-xmax_)*
				(yreg_[npoints_]-yreg_[npoints_-1])/dx_ );
			else if (setzero_)// if automatically set to zero
				return 0;
			}
  	...
  	else   // us the points xs_,ys_ directly 
		{
    		if (x<=xs_[0]) 
    			return ( ys_[0]+(x-xs_[0])*
    					(ys_[1]-ys_[0])/(xs_[1]-xs_[0]) );
    		if (x>=xs_[ksmx_]) 
    			return ( ys_[ksmx_]+(x-xs_[ksmx_])*
    					(ys_[ksmx_]-ys_[ksmx_-1])/
	    	...
	    	while(x>xs_[k]) k++; 
	    		...
	    		if (k>=xs_.size()) 
	    			{  // this should not happen ...
	      			string emsg = " SInterp1D::YInterp() out of ";
	      			emsg+="range k -> BUG in code ";
...
/* --Method-- */
void SInterp1D::DefinePoints(vector<double>& xs,vector<double>& ys,double xmin, 
				double xmax, size_t npt)
{
	...
	for (size_t i=0; i<npoints_; i++)  
		...
		while (x>xs_[k]) k++; // to make sure xs_[k] > x
                                                    ...
                                                    double xmax, int nComments)
{
  ...
  inputFile.close();
  cout << " SInterp1D::ReadYFromFile()/Info: " << cnt << " Y-values read from file " << filename << endl;  
...
{
  os << " ---- SInterp1D::Print() XMin=" << XMin() << " XMax=" << XMax() << " NPoints=" << npoints_ << endl;
