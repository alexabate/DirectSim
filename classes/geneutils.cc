...
double IntegrateFunc(GenericFunc& func,double par, double xmin,double xmax
       ,double perc,double dxinc,double dxmax,unsigned short glorder)
// --- Integration adaptative ---
// Integrate[func[x,a], {x,xmin,xmax}]
// ..xmin,xmax are the integration limits
// ..dxinc is the searching increment x = xmin+i*dxinc
...
////////////////////////////////////////////////////////////////////////////////////
/*
Integration of one dimensioal functions y=f(x) via Gauss-Legendre method.
--> Calculation of coefficients from developpement pour [x1,x2]
| INPUT:
| x1,x2 : limits de l'intervalle (in nbinteg.h -> x1=-0.5 x2=0.5)
| glorder = degree n du developpement de Gauss-Legendre
| OUTPUT:
| x[] = valeur des abscisses ou l'on calcule (dim=n)
| w[] = value of associated coefficients
| REMARQUES:
| - x et w seront dimensionnes up to n.
| - l'integration est rigoureuse si sur l'intervalle d'integration
| la fonction f(x) can be approximated by a polynomial
| of degree 2*m (monome the + haut x**(2*n-1) )
*/
...
//********************* Special functions ************************//
// the functions below are (fairly) blindly copied from Andrew Zentner's fortran code
...
double RootRidder(GenericFunc& func,double x1, double x2, double xacc)
{
