...
ChisqStats::ChisqStats(TVector<r_8> xvals, TVector<r_8> chisq, double dof)
...
int indexmin=NearestIndex(minchisq,Chisq_);
...
for (int i=0; i<xvals_.Size(); i++)
	if(!isinf(logPraw(i))) cnt++;// count number of NOT Infs
	else cout <<"Inf at x="<<xvals_(i)<<endl;
...
InterpXY interplogP(xv,logP,xv(0),xv(ncoarse-1),n);
...
double ymin, ymax;
y.MinMax(ymin,ymax);
int imax=NearestIndex(ymax,y);
if(imax==(n-1))
	cout << "    IMAX IS LAST INDEX! min y="<<ymin<<", max y="<<ymax<<", (imax="<<imax<<")"<<endl;
...
if(imax<(n-1))
	...
	i=NearestIndex(ylevel,yfrompeak);
	plus_=x(i+(imax+1));
	}
	else
		{
