...
//******************** InitialPowerLaw ******************//
//Class for the power law initial spectrum P(k)=Ak^n
...
TransferEH::TransferEH(double h100,double OmegaCDM0,double OmegaBaryon0,double tcmb,bool nobaryon,int lp)
  : lp_(lp)
...
void TransferEH::Init_(void)
{
  ...
  if(lp_) cout<<"h100="<<h100_<<" H0="<<H0<<") Omatter="<<O0_<<" Ocdm="<<Oc_<<" Ob="<<Ob_<<endl;
...
void TransferEH::SetReturnPart(ReturnPart retpart)
// To return only baryon or CDM part part of transfer function
// retpart = ALL: return full transfer function
//         = CDM : return only CDM part of transfer function
//         = BARYON : return only Baryon part of transfer function
// WARNING: only relevant for nobaryon_=false AND nooscenv!=2
{
 retpart_ = retpart;
}

void TransferEH::SetPrintLevel(int lp)
{
 lp_ = lp;
}


double TransferEH::T0tild(double k,double alphac,double betac)
{
  // formula 10 p 608
...
double TransferEH::operator() (double k)
{
...
double TransferTabulate::operator() (double k)
{
 return InterpTab(k,k_,tf_,interptyp_);
}

int TransferTabulate::ReadCMBFast(string filename,double h100,double OmegaCDM0,double OmegaBaryon0)
{
...
{
  if(OmegaMatter0==0.) {
    cout<<"GrowthEH::GrowthEH_Error: bad OmegaMatter0  value : "<<OmegaMatter0<<endl;
    throw ParmError("GrowthEH::GrowthEH_Error:  bad OmegaMatter0  value");
  }
}

GrowthEH::GrowthEH(GrowthEH& d1)
  : O0_(d1.O0_) , Ol_(d1.Ol_)
{
}

GrowthEH::~GrowthEH(void)
{
}

double GrowthEH::operator() (double z)
...
// dy'(dx) = 2*a*dx + b -> for x=x0 on a dy'(0) = b
{
 ...
 double gz  = (*this)(z);
 double dgz1 = (*this)(z1) - gz;
 double dgz2 = (*this)(z2) - gz;
...
// indentical to the above without changing OmegaLambda0
{
...
//********************* GrowthFN ********************//
// new growth function, more general
...
// Destructor
GrowthFN::~GrowthFN(void)
{
}

// DE calculation
double GrowthFN::GrowthDE(double z)
{
  if (!CalcDEDone_)
	throw ParmError("Have not done Dark energy growth calculation!");
  double gro=DEgrofunc_(z);
  return gro;
};

// LCDM calculation: w0=-1, wa=0
double GrowthFN::GrowthLCDM(double z)
// see Formulae A4 + A5 + A6 page 614
...
void GrowthFN::CalcDE(int na)
{
	...
	for (ia=1;ia<na;ia++)// start at index 1, index 0 alread set
		{
...
//********************** PkSpecCalc *********************//
// P(k) calculated from generic initial spectrum, transfer function and growth factor
PkSpecCalc::PkSpecCalc(InitialSpectrum& pkinf,TransferFunction& tf,GrowthFactor& d1,double zref)
...
//********************** PkTabulate *********************//
// P(k) read in from CMBfast or CAMB file
...
PkEH::PkEH(InitialPowerLaw& pkinf,TransferEH& tf,GrowthEH& d1,double zref)
  : pkinf_(pkinf) , tf_(tf) , d1_(d1)
{
  zref_ = zref;
}
...
double PkEH::operator() (double k)
{
  return (*this)(k,zref_);
}

double PkEH::operator() (double k,double z)
{
  double tf = tf_(k);
  double pkinf = pkinf_(k);
  double d1 = d1_(z);
...
PkAltNorm::~PkAltNorm(void) 
{
}

double PkAltNorm::operator() (double k)
{
  return (*this)(k,zref_);
}

double PkAltNorm::operator() (double k,double z)
{

	double OmegaM=tf_.ReturnOmegaM();
	double h = tf_.Returnh100();
	//cout <<"Om="<<OmegaM<<", h="<<h<<endl;

//     transfer function squared, see Eisenstein&Hu 1998
//      tf2=( transfer(kk,0.D0,cosmology(1),cosmology(4),
//     &               cosmology(8),N_m_nu,M_nus) )**2
	double tf = tf_(k);

//      n + 3
//      pwr = cosmology(5)+3.D0
	double pwr  = Power();

//     k in units of Mpc/h / kpivot (??)
//      knorm = cosmology(8)*kk/kpivot
...
//     growth function^2, see Carroll,Press,Turner 92
	double d1 = Gro(z);

//      ggtoday=DEGROWTH(1.D0,cosmology,ncosmos)
//      powfactor=ggtoday/cosmology(1)
//      powfactor=powfactor*powfactor
	double powfactor = PowFactor(OmegaM);

//	cout <<"Apiv="<<Apiv_<<", pwr="<<pwr<<", powfactor="<<powfactor<<", DelRsq="<<DelRsq_<<", d1*d1="<<d1*d1<<", h4="<<h4<<endl;
//	cout <<" z="<<z<<", d1^2="<<d1*d1<<endl;

//     normalised in a funny way: see Smith et al 2003 eqn 23   
//             DELTAK2 = CP   * (knorm**pwr)*  tf2* powfactor*DR2     *gg  /h4   

	/*if (k<0.002&z<1)
		{
	         cout <<"CP="<<Apiv_<<", pwr="<<pwr<<", powfactor="<<powfactor<<endl;
		 cout <<"DR2="<<DelRsq_<<", gg="<<d1*d1<<", h4="<<h4<<", knorm="<<knorm;
		 cout <<", k="<<k<<endl;
		 cout <<"OmegaM="<<OmegaM<<", tf="<<tf<<endl;
		}*/
...
double PkAltNorm::TransF(double k)
{
	return tf_(k);
}

double PkAltNorm::Power()
{
	double pwr  = ns_ +3.;
	return pwr;
}

double PkAltNorm::kNorm(double k, double h)
{
...
double PkAltNorm::hpower4(double h)
{
	return h*h*h*h;
}

double PkAltNorm::Gro(double z)
{
	return d1_(z);
}

double PkAltNorm::PowFactor(double OmegaM)
{
	double d10=d1_.ReturnGrowthZ0();
	double powfactor = (d10/OmegaM)*(d10/OmegaM);
	return powfactor;
}


//******************* VarianceSpectrum ******************//
// Calculation of power spectrum variance
VarianceSpectrum::VarianceSpectrum(GenericFunc& pk,double R,TypeFilter typfilter,double h100)
  : pk_(pk) 
{
  SetRadius(R);
  SetH100(h100);
...
VarianceSpectrum::VarianceSpectrum(VarianceSpectrum& vpk)
  : pk_(vpk.pk_) , R_(vpk.R_) , h100_(vpk.h100_)
{
...
//           = NOFILTER : no filter just integrate spectrum)
// Note:
//   The best approximation of a top-hat filter (R) is a Gaussian filter with (Rg=R/sqrt(5))
{
  typfilter_ = typfilter;
}

void VarianceSpectrum::SetInteg(double dperc,double dlogkinc,double dlogkmax,unsigned short glorder)
// ATTENTION: we don't integrate f(k)*dk but k*f(k)*d(log10(k))
...
// ATTENTION: it's the filter-squared that is returned
{
  // Just integrate the spectrum without filtering
  if(typfilter_ == NOFILTER) return 1.;

  double x2 = x*x;
  // Filter Gaussian G(x) = exp(-x^2/2)
  //        note G(x)^2 = exp(-x^2)
  ...
  // Filter top-hat T(x) = 3*(sin(x)-x*cos(x))/x^3
  // --- Managing the pseudo-divergence for x->0
...
// Return the maximum of the function to integrate
...
// Return "[kmin,kmax]" such that the function to integrate has "f(k) <= high"
//   Search between [kmin,kmax] not logarithmically
// Input:
//     kmin,kmax : search interval
//     eps : precision required on the values kmin et kmax
// Output:
