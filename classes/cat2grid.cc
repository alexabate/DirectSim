...
Cat2Grid::Cat2Grid(SwFitsDataTable& dt,SimpleUniverse& su,RandomGenerator& rg,FitsInOutFile& fos, string ZOCol,string ZSCol,bool RadialZ,double PZErr,bool Print) 
...
// from data table dt
// su:	  class which holds the cosmological parameters and can be used to calculate cosmological quantities
...
// PZErr: size of photometric redshift errors: sigz = PZErr(1+z); if adding Gaussian photo-z errors to OBSERVED z
// Print: if true prints some extra info to screen whilst in constructor
{
	...
	wnrand_=0,nrand_=0; // weigthed/unweighted number of "galaxies" in random grid
...
// if want to do make interpolation table outside Cat2Grid, just use Row2Record, Rec2EuclidCoord functions
Cat2Grid::Cat2Grid(SwFitsDataTable& dt,SimpleUniverse& su,RandomGenerator& rg,SInterp1D dist2z, SInterp1D z2dist,string ZOCol,string ZSCol,bool RadialZ) 
...
// Find min/max redshift
{
	...
	if (AddGaussErr_)
		cout <<"    Remember: adding Gaussian errors, equivalent distance error = "<<PZDerr_<<" Mpc"<<endl;
	...
	cout <<"    Number of observed galaxies = "<<ng<<", Total number of galaxies = "<<ngall_<<endl;
	if (ng>ngall_)
	...
	cout <<"    OBSERVED redshift range (from column "<<ZOCol_<<"): "<<zsmin_<<"<z<"<<zsmax_<<endl;
...
// from specified survey volume
{
	...
	cout <<"    CHECK Cartesian survey boundaries: "<<Xmin_<<"<X<"<<Xmax_<<", "<<Ymin_<<"<Y<"<<Ymax_<<", "<<Zmin_<<"<Z<"<<Zmax_<<endl;
	...
	Nx_=(sa_size_t)floor(Lx); Ny_=(sa_size_t)floor(Ly); Nz_=(sa_size_t)floor(Lz); // integer number cells needed
	...
	cout <<"    Initial survey boundaries: "<<Xmin_<<"<X<"<<Xmax_<<", "<<Ymin_<<"<Y<"<<Ymax_<<", "<<Zmin_<<"<Z<"<<Zmax_<<endl;
...
//	cout <<"    Rounded survey boundaries: "<<Xmin_<<"<X<"<<Xmax_<<", "<<Ymin_<<"<Y<"<<Ymax_<<", "<<Zmin_<<"<Z<"<<Zmax_<<endl;
	...
	zref_ = dist2z(DCref_);
	cout <<"    Redshift of central pixel = "<<zref_<<", found from comoving distance = "<<DCref_<<endl;
...
//	cout <<"    Rounded grid boundaries: "<<Xmin_<<"<X<"<<Xmax_<<", "<<Ymin_<<"<Y<"<<Ymax_<<", "<<Zmin_<<"<Z<"<<Zmax_<<endl;
	...
	Nx_=(sa_size_t)ceil(Lx); Ny_=(sa_size_t)ceil(Ly); Nz_=(sa_size_t)ceil(Lz); // integer number cells needed
	cout <<"    Integer number of cells: Nx="<<Nx_<<", Ny="<<Ny_<<", Nz="<<Nz_<<endl;
	...
	su_.SetEmissionRedShift(zref_);
	double cod =su_.RadialCoordinateMpc(); // radial distance
	cout <<"    Redshift of central pixel = "<<zref_<<", comoving distance @zref = "<<cod<<endl;
...
// Histo object
{
	...
	if(Nfiles<2)
		{
		cout <<"    Read in full catalog redshifts from "<<FullCatFile<<" from column labelled "<<ZCol<<endl;
		...
		cout <<"    Min z of FULL catalog = "<<minzt<<", max z of FULL catalog = "<<maxzt<<endl;
	...
	// if reading from more than one file
		...
		for (int ic=0;ic<Nfiles;ic++)
			...
			cout <<"    Min z of this FULL catalog = "<<minzt<<", max z of this FULL catalog = "<<maxzt<<endl;
	...
	if (DoDebug_)
		...
		cout << "    Cat2Grid::SaveSelecFunc/Info nzFC,nzOC,nzOCsz histos saved to file " << outfile3 << endl;
		}
	...
	if(inp.fail())
		...
		cout << "    Writing to file ..." << outfile.c_str() << " and "<<outfile2<<endl;
	...
	for(long ig=0; ig<ngall_; ig++)
		{
	  	...
	  	Rec2EuclidCoord(grec,x,y,z,redshift);// convert galaxy position into Euclid coord
      	 	selphi = (*selfuncp_)(redshift);// return selection function value at gal redshift
	  	if (selphi<0)// basic check that value isn't crazy
		...
		// ng_ and ngout_ are summed up in here:
	  	AddToCell(x,y,z,selphi); // add galaxy to correct grid pixel 
	  
      		}

	cout <<"    Number of galaxies actually observed = "<< ngo_<<", TOTAL number of galaxies in catalog = "<<ngall_<<endl;
...
// Compute random weighted grid with same selection function as data
{
	...
	wnrand_=wrgals_.Sum();
	cout <<"    number of gals in in grid = "<<ng_<<", number of gals in random grid = "<<nrand_<<endl;
	cout <<"    Number of galaxies in RANDOM weighted grid = "<<wnrand_<<", should be approximately input density*npixels = "<<nc*n_obs_pixels_<<endl;
	cout <<"    Calculated weighted random grid density = "<<wnrand_/n_obs_pixels_<<", input density = "<<nc<<" (should be approximately the same)"<<endl;
	...
	VarianceRandomGrid();
	cout <<"    Mean of weighted random grid should be roughly "<<nc<<" if all pixels are observed, "<<nc*n_obs_pixels_/Npix_<<" if not"<<endl;
	double minv,maxv;
...
// an angle of size theta at distance Dp
{
	...
	cout <<"    Number of pixels which cover "<<theta<<" radians at distance "<<Dp<<" Mpc, = "<<Nx<<endl;
	
	return Nx;
}

vector<double> Cat2Grid::XtractSubArray(TArray<r_8>& nsub,sa_size_t dNp,sa_size_t Np, double theta,int arrayflag)
...
// arrayflag : specifies which array: ngals,wngals,wrgals to return in nsub
{
	...
	cout <<"    Ranges of sub array: (xmin,xmax) = ("<<minxi<<","<<maxxi<<"), (ymin,ymax) = ("<<minyi<<","<<maxyi<<"), (zmin,zmax) = ("<<minzi<<","<<maxzi<<")"<<endl;
	...
	cout << "    min,center,max redshifts of array = "<<minza<<","<<cenza<<","<<maxza<<endl;
...
vector<double> Cat2Grid::XtractSubArray(TArray<r_8>& nsub,long x1,long x2,long y1,long y2,long z1,long z2,int arrayflag)
